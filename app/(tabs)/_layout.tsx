  ...
  if (focussed) {
    return (
      <ImageBackground
        ...
        className="flex flex-row w-full min-w-[102px]  min-h-[50px] mt-4 justify-center  items-center rounded-full overflow-hidden "
      >
        <Image source={icon} tintColor="#151312" className="size-6 " />
        <Text className="text-secondary text-sm ml-2 font-semibold">
          {title}
        </Text>
      </ImageBackground>
    );
    ...
    >
      ...
      <Tabs.Screen
        ...
        options={{
          ...
          headerShown: false,
          tabBarIcon: ({ focused }) => (
            <TabIcon focussed={focused} icon={icons.person} title="Account" />
          ),
        }}
      />
    </Tabs>
